"""
Main pipeline for OCT image classification using radiomics features.
Orchestrates data loading, feature extraction, feature selection, and model training.
Similar to fundus_pipeline.py but configured for OCT data.
"""
import os
import numpy as np
import pickle
...
def main():
    parser = argparse.ArgumentParser(description='OCT Image Classification Pipeline')
    parser.add_argument('--data_dir', type=str, 
                       default='multieye_data/Oct',
                       help='Directory containing train, test folders and ImageData')
    parser.add_argument('--image_dir', type=str,
                       default='multieye_data/Oct/ImageData/oct-filter-448x448',
                       help='Directory containing OCT images')
    parser.add_argument('--extract_features', action='store_true',
                       help='Extract radiomics features (skip if already extracted)')
    parser.add_argument('--features_file', type=str,
                       default='features/oct_train_features.npz',
                       help='Path to save/load extracted features')
    parser.add_argument('--test_features_file', type=str,
                       default='features/oct_test_features.npz',
    ...
    parser.add_argument('--save_model', type=str, default='models/oct_best_model.pkl',
    ...
    print("="*70)
    print("OCT Image Classification Pipeline")
    print("="*70)
    ...
    # For OCT, we only have train and test (no dev set)
    # So we'll use train for training
    from data_loader import load_labels, find_image_file
    
    train_labels_dict = load_labels(str(train_label_file))
    image_path_label_pairs = []
    label_mapping = {}
    
    for img_name, label in train_labels_dict.items():
        img_path = find_image_file(img_name, str(image_dir))
        if img_path:
            image_path_label_pairs.append((img_path, label))
            label_mapping[img_path] = label
    
    print(f"Found {len(image_path_label_pairs)} training images in {time.time() - start_time:.2f}s")
    ...
    if test_label_file.exists():
        ...
        if len(test_path_label_pairs) > 0:
            ...
            if test_features_file.exists() and not args.extract_features:
                ...
                # Save test labels
                test_labels_file = str(test_features_file).replace('.npz', '_labels.npy')
